...
export default function ProductModal({ product, onClose }: ProductModalProps) {
  ...
  const handleShare = async () => {
    ...
    const text = `Olha esse produto que encontrei: ${product.name}`;

    if (typeof navigator.share === 'function') {
      try {
        await navigator.share({ title: product.name, text, url });
      } catch {
        // Usuário cancelou o share nativo — ignorar silenciosamente
      }
    } else {
      try {
        ...
        // Fallback: se clipboard falhar, não faz nada
      }
    }
  };
  ...
  return (
    <Modal isOpen={!!product} onClose={onClose} size="md">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          onClick={handleShare}
          className="flex-shrink-0 mt-1 p-2 rounded-xl border border-[var(--border)] bg-[var(--surface-elevated)] hover:bg-[var(--primary-light)] hover:border-[var(--primary)]/30 transition-all duration-200 group/share"
        ...
        >
          {shareStatus === 'copied' ? (
            ...
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[var(--muted)] group-hover/share:text-[var(--primary)] transition-colors">
      ...
      {shareStatus === 'copied' && (
        <p className="text-xs font-medium text-[var(--primary)] mt-1 animate-fadeIn">Link copiado!</p>
      )}
      {product.description && (
